...
public class LaporanRepository {
    ...
    public List<DataChartBar> getChartStats() {
        ...
        String sql = "SELECT DATE_FORMAT(pemesanan.tanggal, '%M') AS months,SUM(pemesanan.jumlah_bayar) as pemasukann, pengeluaran.total_pengeluaran + (SELECT SUM(pengeluaran_operasional.jumlah) AS pengeluaran_op FROM pengeluaran_operasional WHERE MONTH('2023-06-02') = 6) AS pengeluarann, (SUM(pemesanan.jumlah_bayar) - pengeluaran.total_pengeluaran - (SELECT SUM(pengeluaran_operasional.jumlah) AS pengeluaran_op FROM pengeluaran_operasional WHERE MONTH('2023-06-02') = 6)) AS laba_bersih, (SUM(pemesanan.jumlah_bayar) - pengeluaran.total_pengeluaran) AS laba_kotor FROM `keberangkatan` JOIN pemesanan ON keberangkatan.id = pemesanan.keberangkatan_id JOIN pengeluaran ON keberangkatan.id = pengeluaran.keberangkatan_id JOIN master_paket ON keberangkatan.paket_id = master_paket.id GROUP BY YEAR(pemesanan.tanggal), MONTH(pemesanan.tanggal) LIMIT 5";
